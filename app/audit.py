...
_RULES: list[tuple[str, re.Pattern, str, str]] = [(m, re.compile(p), k, a) for m, p, k, a in [
    ...
    ("POST", r"^/api/v1/projects/(?P<target>[^/]+)/dependencies/edges$", "knowledge", "新增依赖关系"),
    ("POST", r"^/api/v1/projects/(?P<target>[^/]+)/dependencies/edges/[^/]+/confirm$", "knowledge", "确认 AI 识别的依赖"),
    ("DELETE", r"^/api/v1/projects/(?P<target>[^/]+)/dependencies/edges/[^/]+$", "knowledge", "删除依赖关系"),
    ("POST", r"^/api/v1/projects/(?P<target>[^/]+)/dependencies/infer$", "ai", "AI 识别依赖关系"),
    ...
    ("POST", r"^/api/v1/tasks/(?P<target>[^/]+)/cases/import/preview$", "case", "导入预览"),
    ("POST", r"^/api/v1/tasks/(?P<target>[^/]+)/cases/import/confirm$", "case", "确认导入用例"),
    ("POST", r"^/api/v1/tasks/(?P<target>[^/]+)/cases/batch$", "case", "批量维护用例"),
